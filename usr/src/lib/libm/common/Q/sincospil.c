...
#if defined(__LITTLE_ENDIAN) || defined(__i386) || defined(__arm)
#if defined(__arm) && !(HIWORD)
#define	LDBL_MOST_SIGNIF_I(ld)	U(ld, 0)
#define	LDBL_LEAST_SIGNIF_U(ld)	((I(ld, 2) << 16) | (0xffff & (I(ld, 1) >> 15)))
#else
...
void
#if defined(__arm)
__sincospil(long double x, long double *s, long double *c)
#else
