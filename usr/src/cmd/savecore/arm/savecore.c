...
static char 	progname[9] = PROGNAME;
...
static XENV(dumphdr_t) corehdr, dumphdr;/* initial and terminal dumphdrs */
...
read_dumphdr(void)
{
	...
	endoff = llseek(dumpfd, -XENV(DUMP_OFFSET), SEEK_END) &
		-XENV(DUMP_OFFSET);
	Pread(dumpfd, &dumphdr, sizeof (dumphdr), endoff);

	pagesize = dumphdr.dump_pagesize;

	if ((dumphdr.dump_flags & XENV(DF_VALID)) == 0 &&
	    !disregard_valid_flag)
		logprint(-1, verbose, 0, "dump already processed");

	if (dumphdr.dump_magic != XENV(DUMP_MAGIC))
		logprint(-1, verbose, 0, "bad magic number %x",
		    dumphdr.dump_magic);

	if (dumphdr.dump_version != XENV(DUMP_VERSION))
		logprint(-1, verbose, 0,
			 "dump version (%d) != %s version (%d)",
			 dumphdr.dump_version, progname, XENV(DUMP_VERSION));

	if (dumphdr.dump_wordsize != XENV(DUMP_WORDSIZE))
		logprint(LOG_WARNING, 1, 0,
			 "dump is from %u-bit kernel - cannot save on "
			 "%u-bit kernel", dumphdr.dump_wordsize,
			 XENV(DUMP_WORDSIZE));
	/*
	 ...
	 */
	Pread(dumpfd, &corehdr, sizeof (XENV(dumphdr_t)), dumphdr.dump_start);

	corehdr.dump_flags &= ~XENV(DF_VALID);
	dumphdr.dump_flags &= ~XENV(DF_VALID);

	if (memcmp(&corehdr, &dumphdr, sizeof (XENV(dumphdr_t))) != 0) {
		/*
...
static void
build_dump_map(int corefd, const XENV(pfn_t) *pfn_table)
{
	long i;
	static long misses = 0;
	size_t dump_mapsize = (corehdr.dump_hashmask + 1) *
		sizeof(XENV(dump_map_t));
	XENV(mem_vtop_t) vtop;
	XENV(dump_map_t) *dmp = Zalloc(dump_mapsize);
	...
	for (i = 0; i < corehdr.dump_nvtop; i++) {
		...
		Pread(dumpfd, &vtop, sizeof (XENV(mem_vtop_t)),
		      dumphdr.dump_map + i * sizeof (XENV(mem_vtop_t)));
		...
		dmp[i].dm_va = (XENV(uintptr_t))vtop.m_va;
		...
		dmp[h].dm_first = corehdr.dump_map + i *
			sizeof(XENV(dump_map_t));
	}
...
build_corefile(const char *namelist, const char *corefile)
{
	...
	size_t pfn_table_size = dumphdr.dump_npages * sizeof (XENV(pfn_t));
	size_t ksyms_size = dumphdr.dump_ksyms_size;
	size_t ksyms_csize = dumphdr.dump_ksyms_csize;
	XENV(pfn_t) *pfn_table = Zalloc(pfn_table_size);
...
message_save(void)
{
	...
	return 0;
#else	/* !XSAVECORE */
	offset_t dumpoff = -(XENV(DUMP_OFFSET) + XENV(DUMP_LOGSIZE));
	offset_t ldoff;
	XENV(log_dump_t) ld;
	XENV(log_ctl_t) lc;
	...
	dumpfd = Open(dumpfile, O_RDWR | O_DSYNC, 0644);
	dumpoff = llseek(dumpfd, dumpoff, SEEK_END) & -XENV(DUMP_OFFSET);

	ctl.buf = (void *)&lc;
	ctl.len = sizeof (XENV(log_ctl_t));

	dat.buf = Zalloc(XENV(DUMP_LOGSIZE));

	for (;;) {
		ldoff = dumpoff;

		Pread(dumpfd, &ld, sizeof (XENV(log_dump_t)), dumpoff);
		dumpoff += sizeof (XENV(log_dump_t));
		dat.len = ld.ld_msgsize;

		if (ld.ld_magic == 0)
			break;

		if (ld.ld_magic != XENV(LOG_MAGIC))
			logprint(-1, verbose, 0, "bad magic %x", ld.ld_magic);

		if (dat.len >= XENV(DUMP_LOGSIZE))
		...
		Pwrite(dumpfd, &ld, sizeof (XENV(log_dump_t)), ldoff);
	}
	return (0);
#endif	/* XSAVECORE */
}
...
static void
date_format(char *buf, size_t bufsize, time_t t)
{
	struct tm	*tm;

	tm = localtime(&t);
	strftime(buf, bufsize, DATE_FMT, tm);
}

/*
 * static void
 * message_save_file(char *filename)
 *	Save undelivered kernel messages into file.
 *	syslogd(1M) may not be used on ARM architecture, so we should save it
 *	into file.
 */
static void
message_save_file(char *filename)
{
	...
	(void)printf("Constructing message %s/%s\n", savedir, filename);

	fd = Open(filename, O_WRONLY|O_CREAT|O_TRUNC, 0644);
	if ((fp = fdopen(fd, "w")) == NULL) {
		logprint(LOG_ERR, 1, 1, "fdopen(\"%s\"): %s",
	...
	buf = Zalloc(XENV(DUMP_LOGSIZE));

	for (;;) {
		char	datebuf[DATE_BUFSIZE], *p;
		...
		if (len >= XENV(DUMP_LOGSIZE)) {
			logprint(-1, verbose, 0, "bad size %d", ld.ld_msgsize);
		}

		Pread(dumpfd, &lc, sizeof(lc), dumpoff);
		dumpoff += sizeof(lc);

		if (ld.ld_csum != checksum32(&lc, sizeof(lc))) {
			logprint(-1, verbose, 0, "bad log_ctl checksum");
		}

		Pread(dumpfd, buf, len, dumpoff);
		dumpoff += len;

		if (ld.ld_msum != checksum32(buf, len)) {
			logprint(-1, verbose, 0, "bad message checksum");
		}
		*(buf + len) = '\0';

		/* Output timestamp.*/
		date_format(datebuf, sizeof(datebuf), lc.ttime);
		putc('[', fp);
		fputs(datebuf, fp);
		putc(']', fp);
		putc(' ', fp);

		/* Chop off newline characters at the end of line. */
		for (p = buf + len - 1;
		     p >= buf && (*p == '\0' || *p == '\r' || *p == '\n');
		     p--) {
			*p = '\0';
		...
		putc('\n', fp);
		if (ferror(fp)) {
			logprint(LOG_ERR, 1, 1, "fprintf(): %s",
				 strerror(errno));
			/* NOTREACHED */
		}
	}
...
main(int argc, char *argv[])
{
	...
	int mflag = 0;
	long bounds;
	char namelist[30], corefile[30], boundstr[30], msgfile[30];
	time_t	crashtime;
...
#ifdef	XSAVECORE
	if (dumpfile == NULL) {
		logprint(LOG_ERR, 1, 1, "no dumpfile is specified");
	}
#else	/* !XSAVECORE */
	...
	if (dumphdr.dump_panicstring[0] != '\0') {
		...
		XENV(log_ctl_t) lc;
		struct strbuf ctl, dat;
		char msg[XENV(DUMP_PANICSIZE) + 100];
	...
	if ((dumphdr.dump_flags & XENV(DF_COMPLETE)) == 0)
		logprint(LOG_WARNING, 1, -1, "incomplete dump on dump device");

	crashtime = (time_t)dumphdr.dump_crashtime;
	(void) printf("System dump time: %s", ctime(&crashtime));
	...
	(void) sprintf(msgfile, "message.%ld", bounds);

#ifdef	XSAVECORE
	fprintf(stderr, "saving system crash dump in %s/*.%ld",
	    savedir, bounds);
#else	/* !XSAVECORE */
