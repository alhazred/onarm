...
#pragma ident	"@(#)main.c	1.9	07/08/27 SMI"
...
initialize_realms(krb5_context kcontext, int argc, char **argv)
{
	...
	case 'x':
		...
		if( temp == NULL )
		{
		    fprintf(stderr, gettext("%s: KDC cannot initialize. Not enough memory\n"),
			    argv[0]);
		    exit(1);
		}
	...
	case 'r':			/* realm name for db */
		...
		{
			fprintf(stderr, gettext("%s: cannot initialize realm %s. Not enough memory\n"),
				argv[0], optarg);
			exit(1);
		}
	    }
