...
sigvechandler(int sig, siginfo_t *sip, ucontext_t *ucp) 
{
	...
	sc.sc_lr = (int) ucp->uc_mcontext.gregs[REG_LR];
	sc.sc_cpsr = (int) ucp->uc_mcontext.gregs[REG_CPSR];
	...
	ucp->uc_mcontext.gregs[REG_LR] = (int) sc.sc_lr;
	ucp->uc_mcontext.gregs[REG_CPSR] = (int) sc.sc_cpsr;
	/*
	 * Machine dependent code ends
	 */
