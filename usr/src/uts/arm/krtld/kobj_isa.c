...
/*
 * Although kernel module should not contain VFP instructions, gcc does not
 * set SOFT_FLOAT flag into e_flags on EABI mode even if it does not use.
 * All we can verify here is EABI version.
 */
#define	REQUIRED_EFLAGS		(0)
#define	REQUIRED_EABI_VERSION	EF_ARM_EABI_VER4

#else	/* !__ARM_EABI__ */

/* Kernel can't handle VFP instructions. */
#define	REQUIRED_EFLAGS		(EF_ARM_SOFT_FLOAT)
#define	REQUIRED_EABI_VERSION	(0)

#endif	/* __ARM_EABI__ */

#define	HAS_REQUIRED_EABI_VERSION(h)					\
	(((h)->e_flags & EF_ARM_EABIMASK) == REQUIRED_EABI_VERSION)

#define	HAS_REQUIRED_EFLAGS(h)						\
	((((h)->e_flags & REQUIRED_EFLAGS) == REQUIRED_EFLAGS) &&	\
	 HAS_REQUIRED_EABI_VERSION(h))
...
elf_mach_ok(Elf32_Ehdr *h)
{
	return (h->e_ident[EI_DATA] == ELFDATA2LSB &&
		h->e_machine == EM_ARM && HAS_REQUIRED_EFLAGS(h));
}
