...
#ident	"@(#)arm/krtld/kobj_reloc.c"

/*
 * ARM architecture relocation code.
 */
...
/* On STATIC_UNIX environment, executable module is never loaded. */
#ifdef	STATIC_UNIX
#define	IS_KOBJ_EXEC(mp)	(0)
#define	IS_KOBJ_PRIM(mp)	(0)
#else	/* !STATIC_UNIX */
#define	IS_KOBJ_EXEC(mp)	((mp)->flags & KOBJ_EXEC)
#define	IS_KOBJ_PRIM(mp)	((mp)->flags & KOBJ_PRIM)
#endif	/* STATIC_UNIX */
...
extern int	tnf_splice_probes(int boot_load, tnf_probe_control_t *plist,
 ...
 */
static int
tnf_reloc_resolve(struct module *mp, char *symname, Addr *value_p, long offset,
		  tnf_probe_control_t **probelist, tnf_tag_data_t **taglist)
{
...
#define	SDT_NOP		0x01a00000		/* mov{cond} r0, r0 */
...
sdt_reloc_resolve(struct module *mp, char *symname, uint8_t *instr)
{
	...
	ASSERT(IS_P2ALIGNED(instr, sizeof(uint32_t)));
	*((uint32_t *)instr) = (*((uint32_t *)instr) & SDT_COND_MASK) | SDT_NOP;
	    ...
	    int relocsize, Addr baseaddr)
{
	...
	while (reladdr < rend) {
		...
		/*
		 * if R_ARM_RELATIVE, simply add base addr
		 * to reloc location
		 */
		...
		if (rtype == R_ARM_RELATIVE) {
			 ...
			 */
			symref = (Sym *)(mp->symtbl +
					 (stndx * mp->symhdr->sh_entsize));
			...
			if (ELF32_ST_BIND(symref->st_info) == STB_LOCAL) {
				...
				if (symref->st_shndx == SHN_UNDEF &&
				    tnf_reloc_resolve(mp, mp->strings +
						      symref->st_name,
						      &symref->st_value,
						      off, &probelist,
						      &taglist) != 0) {
...
do_relocations(struct module *mp)
{
	...
	for (shn = 1; shn < mp->hdr.e_shnum; shn++) {
		rshp = (Shdr *)(mp->shdrs + shn * mp->hdr.e_shentsize);
		...
		shp = (Shdr *)(mp->shdrs +
			       rshp->sh_info * mp->hdr.e_shentsize);
