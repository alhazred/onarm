...
#define	MUTEX_THREAD_CLRBITS	(~MUTEX_THREAD)
...
#define	MUTEX_DELAY()		mutex_delay()

/*
 * Low overhead clock read to randomize backoff.
 */
extern uint32_t	scucnt_read(void);
#define	MUTEX_GETTICK()		scucnt_read()
...
#define	MUTEX_SYNC()					   \
	do {						   \
		cpuset_t __set;				   \
							   \
		CPUSET_ALL(__set);			   \
		xc_call(0, 0, 0, X_CALL_HIPRI, __set,	   \
