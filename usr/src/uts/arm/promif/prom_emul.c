...
struct nodeq {
	prom_node_t	*nq_node;
	struct nodeq	*nq_next;
};

static prom_node_t *
promif_find_node(pnode_t nodeid)
{
	prom_node_t	*pnp = promif_top, *qnp;
	nodeq_t		*queue;

	if (pnp == NULL || nodeid == OBP_NONODE || pnp->pn_nodeid == nodeid) {
		return pnp;
	}

	/* Root node must have no sibling. */
	ASSERT(pnp->pn_sibling == NULL);

	/*
	 * Search node tree using queue.
	 * We don't want to use recursive call so that we can avoid
	 * stack overflow.
	 */
	queue = kmem_alloc(sizeof(nodeq_t), KM_SLEEP);
	queue->nq_node = pnp->pn_child;
	queue->nq_next = NULL;
	qnp = NULL;
	while (queue != NULL) {
		struct nodeq	*nqp = queue;
		prom_node_t	*np;

		queue = nqp->nq_next;
		pnp = nqp->nq_node;
		kmem_free(nqp, sizeof(nodeq_t));

		for (np = pnp; np != NULL; np = np->pn_sibling) {
			if (np->pn_nodeid == nodeid) {
				qnp = np;
				goto out;
			}
			if (np->pn_child != NULL) {
				nqp = kmem_alloc(sizeof(nodeq_t), KM_SLEEP);
				nqp->nq_node = np->pn_child;
				nqp->nq_next = queue;
				queue = nqp;
			}
		}
...
out:
	while (queue != NULL) {
		struct nodeq	*nqp = queue->nq_next;

		kmem_free(queue, sizeof(nodeq_t));
		queue = nqp;
	}
	return qnp;
}
...
pnode_t
promif_finddevice(char *path)
{
	struct pathname	pn;
	char	*component;
	prom_node_t *pnp;
	pnode_t	nodeid = OBP_BADNODE;

	if ((pnp = promif_top) == NULL) {
		return OBP_BADNODE;
	}
	if (strcmp(path, "/") == 0) {
		return pnp->pn_nodeid;
	...
	while (pn_pathleft(&pn)) {
		...
		i_ddi_parse_name(component, NULL, NULL, &minor);
		while (pnp) {
			char	*nodename, *p;

			if ((nodename = getprop(pnp, "name")) == NULL) {
				continue;
			}

			/* Chop off instance number. */
			if ((p = strrchr(component, '@')) != NULL) {
				*p = '\0';
			}
			if (strcmp(nodename, component) == 0) {
				break;
			}
			pnp = pnp->pn_sibling;
		}
