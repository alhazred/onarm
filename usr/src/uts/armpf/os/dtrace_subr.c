...
{
	(*func)(ARMPF_SYS_VADDR, ARMPF_SYS_SIZE);
	(*func)(ARMPF_SCU_VADDR, ARMPF_SCU_SIZE);

	toxic_range_iterate(func);
}
...
int
dtrace_check_dabttrap(struct regs *rp)
{
	...
	cpu_core[CPU->cpu_id].cpuc_dtrace_illval
	    = (uintptr_t)READ_CP15(0, c6, c0, 0);	/* fault address */
	rp->r_pc += 4;	/* resume at next instruction */
	return (1);
}

/*
 * Additional artificial frames for the machine type. For ARM, we're already
